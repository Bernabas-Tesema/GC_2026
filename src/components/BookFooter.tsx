...
export default function BookFooter({ includeSocials = false }: BookFooterProps) {
    ...
    return (
      <footer className="section-card w-full overflow-hidden">
        <div className="section-body space-y-6 text-center">
          ...
          <p className="border-t border-gold/20 pt-6 text-xs text-navy/45">
  ...
  return (
    <footer className="mt-4 border-t border-gold/25 pt-8">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {footerLinks.map(({ href, icon: Icon, labelKey }) => (
          <Link
            key={href}
            href={href}
            className="nav-btn-hover inline-flex items-center gap-1.5 rounded-full border border-gold/40 bg-paper/80 px-4 py-2 text-xs font-medium text-navy shadow-sm transition-colors hover:bg-gold/10 md:text-sm"
          >
            <Icon className="h-4 w-4 text-gold" />
            {t.nav[labelKey]}
          </Link>
